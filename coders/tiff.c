  ...
  Image *image)
{
  ...
  */
  images=NewImageList();
  AppendImageToList(&images,CloneImage(image,0,0,MagickTrue,&image->exception));
  ...
  while ((columns > 64) && (rows > 64))
  {
    columns/=2;
    rows/=2;
    pyramid_image=ResizeImage(image,columns,rows,UndefinedFilter,image->blur,
      &image->exception);
  ...
  */
  ...
  if (write_info->extract != (char *) NULL)
    write_info->extract=DestroyString(write_info->extract);
