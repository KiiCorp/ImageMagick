  ...
  if (image != (const Image *) NULL)
    {
      ...
      */
      packet_size=3*(image->depth+7)/8;
      if (image->matte != MagickFalse)
        packet_size+=(image->depth+7)/8;
      if (image->colorspace == CMYKColorspace)
        packet_size+=(image->depth+7)/8;
      quantum_info->pixels=(unsigned char *) AcquireQuantumMemory(
        image->columns,packet_size*sizeof(*quantum_info->pixels));
      if (quantum_info->pixels == (unsigned char *) NULL)
        return((QuantumInfo *) NULL);
      quantum_info->extent=packet_size*image->columns*
          ...
          if (quantum_info->min_is_white != MagickFalse)
            {
              ...
              {
                pixel=PushShortPixel(&quantum_state,&p);
                q->red=QuantumRange-ScaleShortToQuantum(pixel);
                q->green=q->red;
                q->blue=q->red;
                p+=quantum_info->pad*sizeof(pixel);
                q++;
  ...
  QuantumInfo *quantum_info)
{
  ...
  quantum_info->scale=(MagickRealType) QuantumRange;
...
%
%      QuantumType GetQuantumType(const Image *image)
%
%  A description of each parameter follows:
%
%    o image: the image.
