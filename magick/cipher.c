...
#define DecipherImageTag  "Decipher/Image "
  ...
  */
  quantum_type=GetQuantumType(image);
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    if ((image->progress_monitor != (MagickProgressMonitor) NULL) &&
      ...
      {
        status=image->progress_monitor(DecipherImageTag,y,image->rows,
          image->client_data);
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    if ((image->progress_monitor != (MagickProgressMonitor) NULL) &&
        (QuantumTick(y,image->rows) != MagickFalse))
      {
        status=image->progress_monitor(EncipherImageTag,y,image->rows,
          image->client_data);
        if (status == MagickFalse)
          break;
      }
...
static void SetAESKey(AESInfo *aes_info,const StringInfo *key)
{
  ...
  */
  ...
  /*
    Generate decipherion key (in reverse order).
  */
  for (i=0; i < 4; i++)
  {
