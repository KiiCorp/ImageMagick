...
#define SpreadImageTag  "Spread/Image"
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    for (x=0; x < (long) image->columns; x++)
    {
      x_distance=(MagickRealType) (width-1.0)*GetPseudoRandomValue();
      y_distance=(MagickRealType) (width-1.0)*GetPseudoRandomValue();
      offset=(MagickOffsetType) ((image->columns+width)*y_distance+x+
        x_distance);
      *q++=(*(p+offset));
    }
